...
class Screen:
    def __init__(self, identifier: ScreenIdentifier,
                 image_loader:callable= None,
                 widget_loder:callable=None,
                 data_loader:callable=None
                 ) -> None:
        self._identifier = identifier
        self._mdi_subwindow = QMdiSubWindow()
        self._mdi_subwindow.setWindowTitle(f"{identifier.value}")
        self._mdi_subwindow.setWindowIcon(QIcon(app_icon))
        self._mdi_subwindow.closeEvent = self.close_event
        self._serialization = None
        self._deserialization = None
        ...
        if widget_loder is not None:
            widget = widget_loder()
            if isinstance(widget, Serializable):
                self._serialization = lambda: widget.serialize()
                self._deserialization= lambda json: widget.deserialize(json)
            self._mdi_subwindow.setWidget(widget)
            if data_loader is not None and self._deserialization is not None:
                self._deserialization(data_loader())
    ...
    def save(self,save_func:callable):
        if self._serialization is not None:
            save_func(self._serialization())
    ...
    def set_title(self, title):
        self._mdi_subwindow.setWindowTitle(f"{self._identifier.value}: {title}")
...
class Conductor:
    ...
    def activate_screen(self, screenIdentifier: ScreenIdentifier,
                        image_loader:callable= None,
                        widget_loader:callable= None,
                        data_loader:callable=None,
                        force = False, loc=None):
        if screenIdentifier not in self._screens or force:
            if force and screenIdentifier in self._screens:
                self._mdi_area.removeSubWindow(self._screens[screenIdentifier].get_window())
            screen = Screen(screenIdentifier,image_loader,widget_loader,data_loader)
        ...
        return lambda title: self._screens[screenIdentifier].set_title(title)

    def save_screen(self,id:ScreenIdentifier, save_func:callable=None):
        if id not in self._screens:
            print("f ScreenIdentifier {id} is not found")
        self._screens[id].save(save_func)
